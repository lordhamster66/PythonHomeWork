...
class AuditLogHandler(object):
    ...
    def __init__(self, log_file, parse_mark):
        self.log_file_obj = AuditLogHandler._get_file(log_file)
        self.parse_mark = parse_mark if parse_mark else "read(5,"

    @staticmethod
    def _get_file(log_file):
    ...
    def parse(self):
        ...
        for line in self.log_file_obj:
            ...
            try:
                ...
                if io_call.startswith(self.parse_mark):
                    if char == '"\\177",':  # 回退
                        char = '[1<-del]'
                    ...
                    if char == '"\\t",':
                        ...
                        continue
                    if char == '"\\r",':
                        cmd_list.append([time_clock, cmd_str.replace("\\r", "")])
                        cmd_str = ''  # 重置
                    if char == '"':  # space
                        cmd_str += ' '
...
if __name__ == "__main__":
    ...
    parser = AuditLogHandler(os.path.join(base_dir, 'log/audit_log/2018-03-11/strace_11.log'))
