...
def upload_img(request):
    ...
    :return:
    """
    file_obj = request.FILES.get("imgFile")  # 获取用户上传的文件对象
    file_suffix = os.path.splitext(file_obj._name)[1]  # 获取文件后缀名
    file_name = "%s%s" % (create_id(), file_suffix)  # 重新构造文件名
    file_path = os.path.join(settings.BASE_DIR, 'static', 'imgs', 'upload', file_name)  # 将文件路径置于服务器
    # 保存用户上传的文件
    with open(file_path, "wb") as f:
        for i in file_obj:
            f.write(i)
    dic = {
        'error': 0,
        'url': '/static/imgs/upload/%s' % file_name,
        'message': ''
    }
...
def file_manager(request):
    ...
    """
    dic = {}
    root_path = os.path.join(settings.BASE_DIR, 'static')
