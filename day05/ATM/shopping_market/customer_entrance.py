...
def shopping(username, goods_list, tmp_buy_dict):
    ...
    while True:
        ...
        while True:
            ...
            print("=========================")
            choice = ["1", "2"]
            pay_choice = input("请选择您的支付方式:").strip()
            if pay_choice in choice:
                break
            else:
                print("输入有误，请重新输入！")
                continue
                ...
                if int(num) <= good_num:
                    ...
                    if pay_choice == "1":
                        if customer_dict[username]["balance"] < total_price:  # 用户余额小于购买商品总价格
                            ...
                            continue
                    if pay_choice == "2":
                        user_info = {"user_dict": {"username": username}}
                        ret = main.consume(user_info, total_price)
                        if not ret:
                            continue
                    ...
                    if date in customer_dict[username]["buy_info"]:             # 同一日期购买
                        ...
                        customer_dict[username]["buy_info"][date][good] = [goods_dict[good][0], int(num)]
                    print("您购买了\033[31;1m%d\033[0m件\033[31;1m%s\033[0m,"
                          "总价格\033[31;1m%d\033[0m,您的账户余额\033[31;1m%d\033[0m"
